...
const CertifiedConcierge: React.FC = () => {
  ...
  // Function to return UI to initial state
  const handleReset = () => {
    setFullName('');
    setEmail('');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase
        ...
        .insert([
          { 
            full_name: fullName, 
            email: email, 
            phone_number: phone, // Added to Supabase payload
            primary_region: selectedState 
          }
        ]);
      ...
      // AUTOMATIC RESET: Return to form after 5 seconds
      setTimeout(() => {
        handleReset();
      }, 5000);
        ...
        <div className="bg-[#0A0D14] rounded-[2rem] md:rounded-[3rem] p-6 md:p-12 lg:p-20 overflow-hidden relative border border-zinc-800/50 shadow-2xl">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center relative z-10">
            ...
            <motion.div 
              ...
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              className="lg:col-span-5 bg-white/[0.03] backdrop-blur-xl p-6 md:p-10 rounded-[2rem] border border-white/10 ring-1 ring-white/5 shadow-2xl min-h-[450px] flex flex-col justify-center"
            >
              <AnimatePresence mode="wait">
                {!submitted ? (
                  ...
                  >
                    <div className="space-y-1">
                      ...
                      <input 
                        ...
                        className="w-full bg-zinc-900/40 border border-white/10 rounded-xl px-5 py-3 text-white placeholder:text-zinc-600 focus:outline-none focus:ring-2 focus:ring-[#1D4ED8]/50 focus:border-[#1D4ED8] transition-all text-sm"
                        required
                      />
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-1">
                        <label className="text-white text-[10px] font-bold uppercase tracking-widest ml-1">Email</label>
                        <input 
                          type="email" 
                          value={email}
                      ...
                      <div className="space-y-1">
                        <label className="text-white text-[10px] font-bold uppercase tracking-widest ml-1">Phone Number</label>
                        <input 
                          type="tel" 
                          value={phone}
                    ...
                    <div className="relative space-y-1">
                      ...
                      <div 
                        ...
                        className={`w-full bg-zinc-900/60 border border-white/10 rounded-xl px-5 py-3 text-white cursor-pointer flex justify-between items-center transition-all hover:bg-zinc-900/80 ${isOpen ? 'ring-2 ring-[#1D4ED8]/50 border-[#1D4ED8]' : ''}`}
                      >
                      ...
                      <AnimatePresence>
                        {isOpen && (
                          <motion.div 
                            ...
                            exit={{ opacity: 0, y: 8 }}
                            className="absolute z-50 w-full mt-2 bg-[#161A22] border border-white/10 rounded-xl max-h-48 overflow-y-auto shadow-[0_20px_50px_rgba(0,0,0,0.5)] custom-scrollbar backdrop-blur-md"
                          >
                            {NIGERIAN_STATES.map((state) => (
                              <div 
                                key={state}
                                onClick={() => { setSelectedState(state); setIsOpen(false); }}
                                className="px-5 py-2.5 text-sm text-zinc-300 hover:bg-[#1D4ED8] hover:text-white cursor-pointer transition-colors border-b border-white/5 last:border-0"
                              >
                                {state}
                              </div>
                            ))}
                    ...
                    <div className="flex items-start gap-3 py-1">
                      <div className="relative flex items-center h-5">
                        <input
                          ...
                          className="w-4 h-4 rounded border-zinc-800 bg-zinc-900 text-[#1D4ED8] focus:ring-[#1D4ED8] focus:ring-offset-zinc-900 accent-[#1D4ED8] cursor-pointer"
                        />
                      </div>
                      <label htmlFor="terms" className="text-zinc-500 text-[10px] leading-snug cursor-pointer select-none">
                        I agree to the Homebiro <span className="text-zinc-300 underline underline-offset-2">Terms</span> and <span className="text-zinc-300 underline underline-offset-2">Privacy Policy</span>.
                      </label>
                    </div>

                    <button 
                      type="submit"
                      disabled={!selectedState || !termsAccepted || isLoading}
                      className="group w-full bg-[#1D4ED8] hover:bg-[#2563EB] disabled:opacity-50 text-white text-sm font-bold py-4 rounded-xl transition-all duration-300 flex items-center justify-center gap-2 shadow-xl shadow-blue-950/20"
                    >
                      {isLoading ? (
                  ...
                  >
                    ...
                    <button 
                      onClick={handleReset}
                      className="text-[#3B82F6] text-[10px] font-bold uppercase tracking-widest hover:text-white transition-colors"
                    >
